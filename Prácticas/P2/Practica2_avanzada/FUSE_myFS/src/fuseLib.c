 ...
 *
 *	struct fuse_file_info{2 pedro pedro      6 oct 30 21:51  file1_lnk.txt
-rw-r--r-- 2
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
    ...
    myFileSystem.nodes[idxNodoI]->links = 1;
...
static int my_unlink(const char *path){
    ...
    int idxNode = myFileSystem.directory.files[idxDir].nodeIdx;
    int links = myFileSystem.nodes[idxNode]->links;
    ...
    if(links == 1){
        ...
        myFileSystem.nodes[idxNode]->links--;
    }
...
static int my_read(const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi){
    ...
    int bytes2Read = size;
    int totalRead = 0;
    char bufAux[BLOCK_SIZE_BYTES];
   
    while (bytes2Read > 0){    
        int currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
        int offBlock = offset % BLOCK_SIZE_BYTES;

        readBlock(&myFileSystem, currentBlock, &bufAux);
        
        int i;
        for(i = offBlock; (i < BLOCK_SIZE_BYTES) && totalRead < size; i++){
            buffer[totalRead] = bufAux[i];
            totalRead++;
        }
...
static int my_link(const char *path1, const char *path2){
    ...
    int idxDir1 = findFileByName(&myFileSystem, (char *)path1+1);
    int idxNode = myFileSystem.directory.files[idxDir1].nodeIdx;
    
    if(idxDir1 == -1){
        return EFAULT;
    }

    // We check that the length of the file name is correctupdateDirectory(&myFileSystem);
    updateNode(&myFileSystem, idxNode, myFileSystem.nodes[idxNode]);
    if(strlen(path2 + 1) > myFileSystem.superBlock.maxLenFileName) {
        return -ENAMETOOLONG;
    }
    ...
    /// Update all the information in the backup file:
    int idxDir2;
    if((idxDir2 = findFreeFile(&myFileSystem)) == -1) {
        return -ENOSPC;
    }


    myFileSystem.directory.files[idxDir2].freeFile = false;
    myFileSystem.directory.numFiles++;
    strcpy(myFileSystem.directory.files[idxDir2].fileName, path2 + 1);
    myFileSystem.directory.files[idxDir2].nodeIdx = idxNode;
    
    myFileSystem.nodes[idxNode]->modificationTime = time(NULL);
    myFileSystem.nodes[idxNode]->links++;
    myFileSystem.nodes[idxNode]->type = LINK;
    updateDirectory(&myFileSystem);
    updateNode(&myFileSystem, idxNode, myFileSystem.nodes[idxNode]);
...
static int my_symlink(const char *path1, const char *path2){
    fprintf(stderr, "--->>>my_symlink path: %s\n", path2);

    // We check that the length of the file name is correctupdateDirectory(&myFileSystem);
    //updateNode(&myFileSystem, idxNode1, myFileSystem.nodes[idxNode1]);
    if(strlen(path2 + 1) > myFileSystem.superBlock.maxLenFileName) {
        return -ENAMETOOLONG;
    }
    ...
    /// Update all the information in the backup file:
    int idxNode2, idxDir2;
    if((idxNode2 = findFreeNode(&myFileSystem)) == -1 || (idxDir2 = findFreeFile(&myFileSystem)) == -1) {
        return -ENOSPC;
    }
    ...
    myFileSystem.directory.numFiles++;
    strcpy(myFileSystem.directory.files[idxDir2].fileName, path2 + 1);
    myFileSystem.directory.files[idxDir2].nodeIdx = idxNode2;
    myFileSystem.numFreeNodes--;

    fprintf(stderr, "El nodo 2 es %i", idxNode2);


    // Fill the fields of the new inode
    if(myFileSystem.nodes[idxNode2] == NULL)
        myFileSystem.nodes[idxNode2] = malloc(sizeof(NodeStruct));


    int numBloques = strlen(path1+1)/BLOCK_SIZE_BYTES;
	if((strlen(path1+1)%BLOCK_SIZE_BYTES )!= 0){
		numBloques = (strlen(path1+1)/BLOCK_SIZE_BYTES)+1;
	}
	myFileSystem.nodes[idxNode2]->type = SYM_LINK;
	myFileSystem.nodes[idxNode2]->fileSize = strlen(path1 + 1);
	myFileSystem.nodes[idxNode2]->numBlocks = numBloques;
	myFileSystem.nodes[idxNode2]->modificationTime = time(NULL);
	myFileSystem.nodes[idxNode2]->freeNode = false;
    myFileSystem.nodes[idxNode2]->links = 1;

	reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNode2]->blocks, numBloques);
	...
	//    symlink
	/*COMPLETAR*/
	my_write(path2, path1, strlen(path1+1), 0, &fi);


    updateDirectory(&myFileSystem);
    updateNode(&myFileSystem, idxNode2, myFileSystem.nodes[idxNode2]);

    return 0;
}

static int my_readlink(const char *path, char *lpath, size_t size)
{
	int idxDir;
	int n;
	int len;
	...
	fprintf(stderr, "--->>>my_readlink: path %s\n", path);
	...
	if (myFileSystem.nodes[n]->type != SYM_LINK)
		return -EINVAL;

	fi.fh = n;

	if ((len = my_read(path, lpath, size, 0, &fi)) < 0)
		return len;
	...
	fprintf(stderr, "Path objetivo: %s\n", lpath);
