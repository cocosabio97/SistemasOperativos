...
static int my_unlink(const char *path){
    ...
    fprintf(stderr, "--->>>my_truncate: path %s", path+1);
...
static int my_read(const char* path, char* buffer, size_t size, off_t offset, struct fuse_file_info* fi) {
	...
	while (bytes > 0) {
		int bloque = node->blocks[offset / BLOCK_SIZE_BYTES];
		int offBloque = offset % BLOCK_SIZE_BYTES;

		readBlock(&myFileSystem, bloque, &bufAux);
		int i = offBloque;
		while (i < BLOCK_SIZE_BYTES && leido < size) {
			buffer[leido] = bufAux[i];
			leido++;
			++i;
		}
