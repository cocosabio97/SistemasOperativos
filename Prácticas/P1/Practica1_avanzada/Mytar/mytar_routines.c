...
int createAlternative(int nFiles, char *fileNames[], char tarName[]){
	...
	tarFile = fopen(tarName, "w");
	header = (stHeaderEntry *)malloc(sizeof(stHeaderEntry) * nFiles);
	fseek(tarFile, 2*sizeof(int), SEEK_SET);
	for(int i = 0; i < nFiles; ++i){
		inputFile = fopen(fileNames[i], "r");
		header[i].size = copynFile(inputFile, tarFile, INT_MAX);
		tamanioTotal += header[i].size;
		fclose(inputFile);
	}
	...
	rewind(tarFile);
	fwrite(&nFiles, sizeof(int), 1, tarFile);
	fwrite(&tamanioTotal, sizeof(int), 1, tarFile);
	fseek(tarFile, tamanioTotal, SEEK_CUR);
	for(int i = 0; i < nFiles; ++i){
		fwrite(header[i].name, sizeof(char), strlen(header[i].name) + 1, tarFile);
		fwrite(&header[i].size, sizeof(header[i].size), 1, tarFile);
	}

	fprintf(stdout, "alternative mtar file created successfully\n"); //Mostramos mensaje de éxito.
...
int extractAlternative(char tarName[]){
	FILE *tarFile, *fich;
	int nFiles, tamanioTotal;
	stHeaderEntry *header;

	tarFile = fopen(tarName, "r");
	fread(&nFiles, sizeof(int), 1, tarFile);
	fread(&tamanioTotal, sizeof(int), 1, tarFile);
	fseek(tarFile, tamanioTotal, SEEK_CUR);
	header = readHeader(tarFile, &nFiles);
	fseek(tarFile, 2*sizeof(int), SEEK_SET);
